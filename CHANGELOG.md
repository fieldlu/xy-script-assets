...
### 新增
- **一键雷达连播**：休眠区新增 🔊一键连播 按钮，自动扫描全网未完成任务，筛选视频/文档类，按 DDL 截止时间智能排序，一键导入调度队列并立即启动连播
- **刷课区雷达连播对齐**：课程区"雷达连播"模式按钮与休眠区一键连播使用统一逻辑

### 修复
- **情报站本地化**：公告不再依赖跨域 CORS 代理（api.allorigins.win / codetabs），改为嵌入式本地数据，彻底解决公告无法加载问题，实现秒开零延迟
